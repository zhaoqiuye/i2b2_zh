...
def make_create_sql():
    ...
    # os.walk返回tupple(dirpath, dirnames, filenames),
    # 其中第一个为起始路径，
    # 第二个为起始路径下的文件夹,
    # 第三个是起始路径下的文件.
    for mdb_file in os.walk(dir):
        if len(mdb_file[2]) > 0:
            for file_p in mdb_file[2]:
                if file_p[-3:] == 'mdb':
                    print file_p
                    cmd = 'mdb-schema %s  >>/usr/dataDir/create.sql'
                    cmd = cmd % (dir + file_p)
                    print cmd
                    os.system(cmd)
...
def modefy_create_sql():
    ...
    for eachline in fobj:
        if eachline.find('[') > 0:
            ...
            #将postgresql不支持的类型替换
            if eachline.find('Long Integer'):
                eachline=eachline[:].replace('Long Integer','Integer')
            if eachline.find('Memo/Hyperlink (255)'):
                eachline=eachline[:].replace('Memo/Hyperlink (255)','varchar(2000)')
            if eachline.find('DateTime'):
                eachline=eachline[:].replace('DateTime','varchar(255)')
            if eachline.find('Double'):
                eachline=eachline[:].replace('Double','varchar(255)')
        # 判断表名中是否含有空格
...
def make_insert_csv():
    for file_p in mdb_tbl_dic.keys():
        for tbl in mdb_tbl_dic[file_p]:
            if len(tbl) > 0:
                cmd = 'mdb-export    %s %s > %s.csv' % (
                dir + file_p, '"' + tbl + '"', dir + '"' + tbl + '"')  # tbl.replace(' ', '_').replace('&', '_'))
                os.system(cmd)
...
def insert_into_database():
    cmd = 'psql -h localhost -d test -U postgres -f %s 2>>log.txt' % (dir + 'create.sql')
    os.system(cmd)
    ...
    for sql_file in os.walk(dir):
        if len(sql_file[2]) > 0:
            for file_p in sql_file[2]:
                print file_p
                if file_p[-3:] == 'csv':
                    cmd = 'psql -h localhost -d test -U postgres -f %s 2>>log.txt' % (dir + '"' + file_p + '"')
                    os.system(cmd)
